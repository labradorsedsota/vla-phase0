...
每步截图必须本地存储，按步序命名：`trajectories/<app>-<golden|buggy>/screenshots/<测试点ID>/step_N.png`

mano-cua 当前将截图存储于云端，本地不自动落盘。需在采集完成后从云端拉取，或推动 mano-cua 增加本地截图输出参数。截图缺失的轨迹不可离线审计。
